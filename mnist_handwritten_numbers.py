# -*- coding: utf-8 -*-
"""MNIST_handwritten_numbers_v2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/15USj7JQlkpgVNNzs2Cw1wjrWjoDoQH_4

# Welcome to image recognition tutorial

Once you see this, means that you successfully downloaded, installed and run jupyter notebooks. 
Jupyter notebook is just a web based solution to run regular python scripts, with the benefits of adding text inbetween.


In this tutorial we will work with probably the most familiar dataset in image classification - [MNIST data set](http://yann.lecun.com/exdb/mnist/). MNIST is a labeled set of images of handwritten digits. The tutorial takes advantage of the Tensorflow which is Googles open source machine learning library.  

An example follows of the digits we will classify:
"""
...
We'll proceed in steps, beginning with importing the MNIST data. This doesn't have anything to do with TensorFlow in particular -- we're just downloading the data archive.
"""
...
# TASK 1 - Segmenting data into training, test, and validation

The final step in preparing our data is to split it into three sets: training, test, and validation. This isn't the format of the original data set, so we'll take a small slice of the training data and treat that as our validation set.
"""

"""
Choose a validation size for which the neural network will use to validate data when training:
The bigger validation size, the less training data the network gets.
- And opposite, the lesser validation size, the more training data.
"""
VALIDATION_SIZE = # Choose the number of images to keep for validation  [1000-20000]
...
This is where Tensorflow library comes in.
This tutorial uses some handy approaches within image recognition, like `convolution`, `max pooling`, `rectified linear`and so on.
...
# TASK 2 - Decide size for each layer
"""
...
""" Decide number of nodes for layer 1, 2 and 3. Stick to powers of 2 (2, 4, 8, 16, etc)"""
LAYER_1 = # Choose a node count for layer 1  [1-128]
LAYER_2 = # Choose a node count for layer 2  [1-128]
LAYER_3 = # Choose a node count for layer 3  [1-1024]
...
conv1_weights = tf.Variable(
  tf.truncated_normal([5, 5, NUM_CHANNELS, LAYER_1],
                      stddev=0.1,
                      seed=SEED))
conv1_biases = tf.Variable(tf.zeros([LAYER_1]))
conv2_weights = tf.Variable(
  tf.truncated_normal([5, 5, LAYER_1, LAYER_2], 
                      stddev=0.1,
                      seed=SEED))
conv2_biases = tf.Variable(tf.constant(0.1, shape=[LAYER_2]))
fc1_weights = tf.Variable(
  tf.truncated_normal([IMAGE_SIZE // 4 * IMAGE_SIZE // 4 * LAYER_2, LAYER_3],
                      stddev=0.1,
                      seed=SEED))
fc1_biases = tf.Variable(tf.constant(0.1, shape=[LAYER_3]))
fc2_weights = tf.Variable(
  tf.truncated_normal([LAYER_3, NUM_LABELS],
                      stddev=0.1,
                      seed=SEED))
...
"""### The neural network now looks like

### Input Layer --> Layer 1 --> Layer 2 --> Layer 3 --> 10 output nodes: The labes 0 to 9.
...
The learning rate decides how fast the neural network should learn. 

With a high learning rate (> 0.1), the network learns really fast - but is not too precise
Lowering the learning rate (i.e. 0.01) makes the learning process slower, but more precise.
"""

""" 
    Choose a learning rate
    Everything between 0.001 and 0.2 may be tested
"""
PICK_LEARNING_RATE = # Choose a learning rate
...
learning_rate = tf.train.exponential_decay(
  PICK_LEARNING_RATE,  # Base learning rate.
...
# TASK 4 - Improve result by changing parameters

Experiment with the parameters you have defined in the previous tasks. See if you can improve accuracy.

# TASK 5 - Improve result by training multiple times
...
# TASK 6 - Try with your own images!

1.   Draw a digit in paint (black digit on white background)
2.   Save it as a .png file
3.   Upload it
4.   Run the code block below (The code expects to find the image in sample_data/myDigit.png, but you can change that if you want)
"""
...
img = np.invert(Image.open('sample_data/myDigit.png').convert("L"))
...
my_data_node = tf.constant(im2arr)
my_prediction = tf.nn.softmax(model(my_data_node))
predictions = my_prediction.eval()
